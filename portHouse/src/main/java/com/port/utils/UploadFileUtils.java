...
public class UploadFileUtils {
	...
	public static String calcPath(String uploadPath) {
		Calendar cal = Calendar.getInstance(); // 캘린더를 불러옴
		String yearPath = File.separator + cal.get(Calendar.YEAR); // 캘린더에서 연도만 추출
		// 캘린더에서 월만 추출. 형식은 00(01월, 02월...11월, 12월) 단, 1월은 캘린더상에서 0이므로, +1을 해줌
		String monthPath = yearPath + File.separator + new DecimalFormat("00").format(cal.get(Calendar.MONTH) + 1);
		String datePath = monthPath + File.separator + new DecimalFormat("00").format(cal.get(Calendar.DATE)); // 캘린더에서 날짜만 추출
		
		makeDir(uploadPath, yearPath, monthPath, datePath); // 원본 이미지가 저장될 경로 생성
		makeDir(uploadPath, yearPath, monthPath, datePath + "\\s"); // 썸네일 이미지가 저장될 경로 생성
